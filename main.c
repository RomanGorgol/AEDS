...
int main(int argc, char* argv[]) {
	...
	long lN = config.N, lM = config.M;
	long amount = (lN*(lM+2*mpi.process_count))/mpi.process_count;
	if (amount > INT_MAX)
	{
		if (mpi.rank == 0)
		{
...
//		}
	...
	double* U;
	ind_t sizeU = (config.N+2*mpi.process_count)*config.M;
	if (mpi.rank == 0)
	{
